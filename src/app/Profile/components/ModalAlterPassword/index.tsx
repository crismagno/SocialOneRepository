...
const ModalAlterPassword: React.FC<IModalAlterPasswordProps> = (
  ...
  const [lastPasswordIsValid, setLastPasswordIsValid] = useState<boolean>(
    false,
  );
  ...
  const validateLastPassword = async (): Promise<void> => {
    try {
      setLoad(true);
      const response = await userService.validatePassword(
        user?._id,
        lastPassword,
      );
    ...
    <Modalize
      ...
      <View style={styles.container}>
        ...
        <ButtonGradient
          ...
          onPress={() =>
            lastPasswordIsValid ? updatePassword : validateLastPassword()
          }
