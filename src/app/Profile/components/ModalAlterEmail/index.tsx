...
import {user as userService, code as codeService} from './../../../../services';
...
const ModalAlterEmail: React.FC<IModalAlterEmailProps> = (
  ...
  const [loadCancelEmailPending, setLoadCancelEmailPending] = useState<boolean>(
    false,
  );

  const validateExistsEmailChange = async (): Promise<void> => {
    try {
      const validateExistsEmailChange = await userService.validateExistsEmailChange(
        user._id,
      );
      ...
      <View style={styles.container}>
        ...
        <If condition={!loadCancelEmailPending}>
          ...
          <ButtonGradient
            ...
            onPress={() =>
              !existsEmailToChange ? changeEmail() : validateCodeEmail()
            }
