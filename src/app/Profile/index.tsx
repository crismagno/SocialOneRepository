...
const Profile: React.FC<any> = (props): JSX.Element => {
  ...
  const [showModalAlterFullName, setShowModalAlterFullName] = useState<boolean>(false);
  const [showModalAlterEmail, setShowModalAlterEmail] = useState<boolean>(false);
  const [showModalAlterPhone, setShowModalAlterPhone] = useState<boolean>(false);
  const [showModalAlterPassword, setShowModalAlterPassword] = useState<boolean>(false);
  ...
  const onLogout = async () => {
    try {
      setShowLogout(true);
      execActionsNavigation.resetHistory("SignIn");
